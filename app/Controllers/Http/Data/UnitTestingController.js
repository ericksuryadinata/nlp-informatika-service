    ...
    try {
      ...
      if (intent === 'cariJadwalDosenGeneral') {
        ...
        if (jadwal == null || jadwal.toJSON().length === 0) {
          ...
          for (const element of jadwal) {
            ...
            let jamKode = element.jam.split('-')
            let mengajarJamKe = ''
            let jam = await Jam.query().where('kode', jamKode[0]).first()
            mengajarJamKe = jam.jam_kuliah_masuk + '-'
            jam = await Jam.query().where('kode', jamKode[1]).first()
            mengajarJamKe = mengajarJamKe + jam.jam_kuliah_keluar
            result += mataKuliah.nama + ' Hari ' + hari.nama + ' Kelas ' + element.kelas + ' Ruang ' + element.ruang + ' Jam : ' + mengajarJamKe + '\r\n'
          }
        }
      }
      ...
      if (intent === 'cariPraktikumNama') {
        ...
        if (kelasLaboratorium == null) {
          ...
          for (const element of kelasLaboratorium) {
            ...
            jam = await Jam.query().where('kode', jamKode[1]).first()
            mengajarJamKe = mengajarJamKe + jam.jam_kuliah_keluar
            result += 'Kelas ' + element.nama + ' hari ' + hari.nama + ' Jam : ' + mengajarJamKe + '\r\n'
          }
        }
      }
      ...
      if (intent === 'cariNilaiPraktikumNama') {
        if (typeof entities.nbi === 'undefined') {
          ...
          const nilaiPraktikum = await KelasLaboratoriumMahasiswa.query()
            .innerJoin('kelas_laboratorium', 'kelas_laboratorium_mahasiswa.kelas_laboratorium_id', 'kelas_laboratorium.id')
            .innerJoin('praktikum_laboratorium', 'kelas_laboratorium.praktikum_laboratorium_kode', 'praktikum_laboratorium.kode')
            .where('kelas_laboratorium_mahasiswa.nbi', entities.nbi)
            .where('praktikum_laboratorium.kode', praktikumLaboratorium.kode)
            .orderBy('kelas_laboratorium_mahasiswa.akhir').first()
          if (nilaiPraktikum.length === 0 || nilaiPraktikum === null) {
            result = entities.nbi + ' tidak mengikuti praktikum ' + praktikumLaboratorium.nama
          } else {
            result = answer + ' ' + nilaiPraktikum.grade
          }
        }
      }
      ...
      if (intent === 'sapaanKabar') {
        result = answer
      }

      if (intent === 'sapaanKabarTanya') {
        result = answer
      }
